...
DB_HOST=postgres
DB_PORT=5432
DB_DATABASE=nutrikids
DB_USERNAME=nutrikids_user
DB_PASSWORD=${POSTGRES_PASSWORD}
```

`DB_PASSWORD` debe coincidir con `POSTGRES_PASSWORD`, la variable que consume
`docker-compose.yml` para crear el contenedor `nutrikids_postgres`.
